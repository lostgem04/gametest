...
class UI:
    ...
    def _draw_map(self, buf):
        ...
        # ── posiciones de ghosts (otros jugadores, mismo mundo) ───────────────
        ghost_cells = {}
        for g in getattr(w, '_ghost_players', []):
            if not g.get('alive', True):
                continue
            gx, gy = int(g.get('x', -1)), int(g.get('y', -1))
            if (gx, gy) not in ghost_cells:
                try:
                    from races import RACE_DEFS
                    rc = RACE_DEFS.get(g.get('race_id', 'human'), {}).get('color', (100, 200, 255))
                except Exception:
                    rc = (100, 200, 255)
                initial = (g.get('name', '?') or '?')[0].upper()
                ghost_cells[(gx, gy)] = (fg(*rc), initial)

        # ── posiciones de animales ────────────────────────────────────────────
        animal_cells = set()
        animal_mgr = getattr(w, 'animal_manager', None)
        if animal_mgr:
            for a in animal_mgr.animals:
                if a.get('alive', True):
                    animal_cells.add((int(a['x']), int(a['y'])))
        ...
        for rel_y in range(-hy, hy+1):
            ...
            for rel_x in range(-hx, hx+1):
                ...
                if not w.revealed[my][mx]:
                    line.append(' ')
                    continue
                has_ghost  = (mx, my) in ghost_cells
                has_boss   = any(int(e['x'])==mx and int(e['y'])==my and e['alive'] and e.get('boss') for e in w.enemies)
                has_e      = any(int(e['x'])==mx and int(e['y'])==my and e['alive'] for e in w.enemies)
                has_i      = (mx, my) in w.items
                has_v      = (mx, my) in getattr(w, 'ore_veins', {}) and w.ore_veins[(mx,my)]['charges'] > 0
                has_animal = (mx, my) in animal_cells

                if has_ghost:
                    gcol, initial = ghost_cells[(mx, my)]
                    line.append(gcol+BOLD+initial+RESET)
                elif has_boss:
                    ...
                    line.append(fg(180,140,80)+'a'+RESET)
                else:
                    try:
    ...
    def hud_equipped_hand(self, buf, player, anim_state='idle', anim_tick=0):
        ...
        Los sprites están definidos en sprites.py — get_hand_sprite().
        Los espacios son transparentes (no sobreescriben el mundo 3D).
        """
        from sprites import get_hand_sprite

        vw = self.L.view_w
        vh = self.L.view_h

        art, anim_color, offset_row = get_hand_sprite(player, anim_state)
